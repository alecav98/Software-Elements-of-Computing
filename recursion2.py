...
def groupSum6(start, nums, target):
  if (target == 0) and (start == len(nums)):
    ...
    return False
  elif (nums[start] == target) and (6 not in nums[start:]):
    return True
  elif (nums[start] == 6):
...
def groupSum5(start, nums, target):
  if (target == 0) and (start == len(nums)):
    ...
    return False
  elif (nums[start] == target) and (5 not in nums[start:]):
    return True
  elif (nums[start]%5 == 0):
    if start+1 >= len(nums):
...
def groupSumClump(start, nums, target):
  if (target == 0):
    ...
    count = 1
    start2 = start
    while nums[start2] == nums[start2+1]:
      count += 1
      start2 += 1
...
def splitArrayHelper(a, b, c, idx):
  if (idx == len(a)):
    ...
    d = b[:]
    b.append(a[idx])
    splitArrayHelper(a, b, c, idx+1)
    splitArrayHelper(a, d, c, idx+1)
...
def split53(nums):
  ...
  tot1 = 0
  tot2 = 0
  return split53Helper(nums, idx, tot1, tot2)

def split53Helper(nums, idx, tot1, tot2):
  if (idx >= len(nums)):
    return tot1 == tot2
  val = nums[idx]
  if (val%5 == 0):
    return split53Helper(nums, idx + 1, tot1+val, tot2)
  elif (val%3 == 0):
    return split53Helper(nums, idx + 1, tot1, tot2+val)
  else:
    return(split53Helper(nums, idx + 1, tot1+val, tot2) or split53Helper(nums, idx + 1, tot1, tot2+val))
